  ...
  <article class="card station-card">
    ...
    <div class="sensor-preview">
      ...
      <div v-if="sensors.length" class="sensor-preview__list">
        <div v-for="sensor in sensors.slice(0, 3)" :key="sensor.sensor_id" class="sensor-preview__item">
          <span>Датчик №{{ sensor.sensor_id }}</span>
          <small>{{ sensorSummary(sensor) }}</small>
        </div>
        <small v-if="sensors.length > 3" class="muted">+{{ sensors.length - 3 }} ещё</small>
      </div>
      <p v-else class="muted">Данные от почвенных датчиков пока не поступали.</p>
    </div>

    <RouterLink v-if="station?.field_id" :to="stationLink" class="btn btn--primary station-card__button">
      Открыть станцию
    </RouterLink>
    <span v-else class="hint">Не удалось открыть станцию: нет данных о привязанном поле.</span>
  </article>
</template>
